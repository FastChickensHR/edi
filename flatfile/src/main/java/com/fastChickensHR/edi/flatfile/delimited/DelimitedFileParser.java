...
/**
 * The delimited flat-file implementation of the {@link FileParser} seam: reads a header-row
 * delimited file (CSV by default, or any {@link DelimitedFormat}) into a format-neutral {@link
 * FileContent}. A delimited file is the <em>Flat</em> shape — one line per record — so each data
 * row becomes one {@link Record} and each non-empty cell becomes a {@link Field} whose {@link
 * Location} name is the column name.
 *
 * <p>The parser is intentionally dumb: it knows columns and cells, not domain meaning. In a plain
 * flat file every row is a top-level {@link Record} whose fields sit at {@link RecordLevel#RECORD}
 * — the consuming application assigns each column's data element downstream. When the reserved
 * {@link LinkedRows#RECORD_LEVEL_COLUMN} is present, the parser reconstructs nesting: {@code
 * SUBRECORD} rows attach as children of the {@code RECORD} row they follow, inverting {@link
 * DelimitedFileGenerator}. Empty cells produce no field (absence, not a blank value).
 *
 * <p><b>Headerless formats</b> ({@link DelimitedFormat#hasHeader()} {@code == false}) have no
 * column names to read, so cells are addressed by <em>position</em>: the first column is {@code
 * "1"}, the second {@code "2"}, and so on — 1-based, matching the element numbering the rest of
 * this library speaks. A headerless file carries no {@link LinkedRows#RECORD_LEVEL_COLUMN} to
 * recognise either, so it always parses flat; reconstructing nesting requires a header row.
 */
public final class DelimitedFileParser implements FileParser {
  ...
  private static List<Record> parseNested(CSVParser parser, List<String> headers) {
    ...
    for (CSVRecord row : parser) {
      String level =
          row.isMapped(LinkedRows.RECORD_LEVEL_COLUMN)
              ? row.get(LinkedRows.RECORD_LEVEL_COLUMN)
              : "";
