...
class STSegmentTest {
  ...
  void testCreation_WithEnumValue_ShouldCreateSegment() throws ValidationException {
    TransactionSetIdentifierCode code =
        TransactionSetIdentifierCode.BENEFIT_ENROLLMENT_AND_MAINTENANCE;
    String transactionSetControlNumber = "0001";
    ...
    assertEquals(transactionSetControlNumber, segment.getTransactionSetControlNumber());
    assertNull(
        segment.getImplementationConventionReference(),
        "Implementation convention reference should be null when not set");
  }
  ...
  @Test
  void testToString_WithOptionalFieldsOmitted_ShouldReturnFormattedString()
  ...
  @NullAndEmptySource
  @ValueSource(strings = {"   ", "\t"})
  void testValidation_InvalidTransactionSetIdentifierCode_ShouldThrowException(
      String invalidValue) {
    STSegment.Builder builder = new STSegment.Builder().setTransactionSetControlNumber("0001");

    Exception exception =
        assertThrows(
            IllegalArgumentException.class,
            () -> builder.setTransactionSetIdentifierCode(invalidValue));
    assertTrue(
  ...
  void testValidation_InvalidTransactionSetControlNumber_ShouldThrowException(String invalidValue) {
    ...
    assertTrue(
        exception.getMessage().contains("Transaction Set Control Number")
  ...
  void testValidation_TransactionSetIdentifierCode_ShouldThrowException() {
    ...
    Exception exception =
        assertThrows(
            IllegalArgumentException.class, () -> builder.setTransactionSetIdentifierCode("12345"));

    assertTrue(
        exception.getMessage().contains("Invalid Transaction Set Identifier Code value"),
        "Exception message should mention the invalid field");
  }
