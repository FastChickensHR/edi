 ...
 * application's persisted {@code Location.name}s and this library's generator: a Java constant may be
 * renamed freely, but changing the <em>string</em> it holds silently orphans every value already
 ...
 * untouched and passes; editing a literal (or adding/removing a key) changes the set and fails.
 * A deliberate vocabulary change is made by regenerating with {@code -Dupdate.goldens=true} and
...
class X834LocationGoldenTest {
    ...
    void everyRenderKeyLiteralMatchesTheGolden() {
        ...
        assertEquals(keys.size(), literals.size(),
                "two X834Location constants carry the same literal — every key must name a distinct location");
        TestFixtures.assertMatchesGolden("golden/x834-location-keys.txt", String.join("\n", literals) + "\n");
    }
    ...
    void indexHelperFormatsAreFrozen() {
        ...
        assertEquals("disability.1.startDate", X834Location.disability(1, X834Location.DISABILITY_START_DATE));
