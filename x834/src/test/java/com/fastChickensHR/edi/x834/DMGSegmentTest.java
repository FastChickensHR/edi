...
class DMGSegmentTest {
   ...
   * carrying only the first two values renders {@code DMG*D8*19900101~} — no eight empty {@code *}
   * placeholders — which the getter/array-length assertions could not distinguish from a
   * fully-padded segment.
   */
  @Test
  void rendersDmgSegmentTrimmingTrailingEmptyElements() throws ValidationException {
    DMGSegment segment = TestDMGSegment.builder().setDmg01("D8").setDmg02("19900101").build();
    segment.setContext(new X834Context());
  ...
  void testBuilderChaining() throws ValidationException {
    DMGSegment segment =
        TestDMGSegment.builder().setDmg01("D8").setDmg02("19900101").setDmg03("M").build();
