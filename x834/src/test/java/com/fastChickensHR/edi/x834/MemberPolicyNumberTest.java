...
class MemberPolicyNumberTest {
  ...
  public void testGetSegmentIdentifierReturnsExpectedValue() throws ValidationException {
    MemberPolicyNumber segment =
        MemberPolicyNumber.builder().setRef02(referenceIdentification).build();
    segment.setContext(context);

    assertEquals(
        "REF", segment.getSegmentIdentifier(), "Expected segment identifier should be 'REF'");
    assertEquals(
        "REF*1L*POLICY123456~", segment.render().trim(), "The segment is not formatted correctly.");
  }
  ...
  public void testValidationRejectsEmptyRef02() {
    ValidationException exception =
        assertThrows(
            ValidationException.class,
            () -> {
              MemberPolicyNumber.builder().setRef02("").build();
            },
