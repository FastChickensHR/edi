...
class INSSegmentTest {
  ...
  void testGetSegmentIdentifierReturnsExpectedValue() throws ValidationException {
    ...
    assertEquals(
        "INS", segment.getSegmentIdentifier(), "Expected segment identifier should be 'INS'");
    ...
    // qualifier (D8).
    assertEquals(
        "INS*Y*18*001*20*A*D*1*FT*F*N*D8*20250101~",
        segment.render().trim(),
  ...
  void testSettingSpecNamesGettingDomainNames() throws ValidationException {
    ...
    assertEquals(
        "18",
        segment.getIndividualRelationshipCode(),
        "IndividualRelationshipCode should match INS02");
    assertEquals("001", segment.getMaintenanceTypeCode(), "MaintenanceTypeCode should match INS03");
    assertEquals(
        maintenanceReasonCode,
        segment.getMaintenanceReasonCode(),
        "MaintenanceReasonCode should match INS04");
    ...
    assertEquals(
        "1", segment.getCOBRAQualifyingEventCode(), "COBRAQualifierCode should match INS07");
    assertEquals(
        employmentStatusCode,
        segment.getEmploymentStatusCode(),
        "EmploymentStatusCode should match INS08");
    ...
    assertEquals(
        "D8", segment.getDateTimePeriodFormatQualifier(), "Format qualifier should match INS11");
    assertEquals("20250101", segment.getDeathDate(), "DeathDate should match INS12");
    assertEquals(
        confidentialityCode,
        segment.getConfidentialityCode(),
        "ConfidentialityCode should match INS13");
  }
  ...
  void testSettingDomainNamesGettingSpecNames() throws ValidationException {
    ...
    assertEquals(
        maintenanceReasonCode, segment.getIns04(), "MaintenanceReasonCode should match INS04");
    ...
    assertEquals(
        employmentStatusCode, segment.getIns08(), "EmploymentStatusCode should match INS08");
    ...
    assertEquals(
        "D8",
        segment.getIns11(),
        "Format qualifier (defaulted from death date) should match INS11");
