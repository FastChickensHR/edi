...
class X834ContextTest {
  ...
  private X834Context validContext() {
    return new X834Context().setInterchangeControlNumber("000000001").setGroupControlNumber("1");
  }
  ...
  void testFormattedDateAndTimeBothFollowSetDocumentDate() {
    X834Context ctx = new X834Context().setDocumentDate(LocalDateTime.of(2024, 1, 1, 0, 0));

    assertEquals(
        "20240101",
        ctx.getFormattedDocumentDate(),
        "ISA09/GS04 date should come from the configured document date");
    assertEquals(
        "0000",
        ctx.getFormattedDocumentTime(),
  ...
  void testFormattedTimeIsNotFrozenAtConstruction() {
    X834Context ctx = new X834Context().setDocumentDate(LocalDateTime.of(2023, 11, 15, 12, 30, 45));
  ...
  void testFormatChangesAfterConstructionAreReflectedOnRead() {
    X834Context ctx = new X834Context().setDocumentDate(LocalDateTime.of(2023, 11, 15, 12, 30, 0));
