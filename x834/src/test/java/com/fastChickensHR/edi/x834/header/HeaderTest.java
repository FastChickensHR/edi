...
class HeaderTest {
   ...
   * rendered in order into one string and compared against an on-disk golden. Whole-payload
   * equality pins segment order, element positions, and each segment's terminator at once — and,
   * because this {@code context} uses the pipe element separator, proves that separator propagates
   * into every rendered segment. The document date is pinned via the context, so the golden is
   * deterministic.
   *
   * <p>Regenerate after an intentional format change with {@code -Dupdate.goldens=true} (see {@link
   * TestFixtures}).
   */
  @Test
  void rendersFullHeaderPayload() throws ValidationException {
    Header header =
        new Header.Builder(context)
