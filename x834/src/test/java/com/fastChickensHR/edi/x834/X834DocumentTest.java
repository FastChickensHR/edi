...
class X834DocumentTest {
  ...
  void multipleMinimalMembersRenderExpected834() {
    ...
    // GE01 and IEA01 stay at 1 — one transaction set inside one functional group inside one
    // interchange.
    X834Document doc =
        new X834Document.Builder(context)
            .withHeader(buildHeader())
  ...
  void missingTrailerFailsWithBuildErrorLocatedAtTrailer() {
    ...
    assertTrue(
        errors.stream().anyMatch(e -> e.phase() == Phase.BUILD && e.location().equals("Trailer")),
  ...
  void rejectsElementSeparatorInAValue() {
    ...
    List<GenerationError> errors = assertFailure(doc.generateDocument());
    assertTrue(
        errors.stream().allMatch(e -> e.phase() == Phase.RENDER),
        "delimiter violations are render-phase");
    assertTrue(
        errors.stream().anyMatch(e -> e.message().contains("ACME*CORP")),
        "an error should name the offending value");
    assertTrue(
        errors.stream().anyMatch(e -> e.message().contains("element separator")),
  ...
  void rejectsSegmentTerminatorInAValue() {
    ...
    List<GenerationError> errors = assertFailure(doc.generateDocument());
    assertTrue(
        errors.stream().anyMatch(e -> e.message().contains("segment terminator")),
        "an error should name the delimiter");
  }
  ...
  void rejectsLowerCaseWhenTheInterchangeIsNarrowedToTheBasicSet() {
    ...
    assertTrue(
        errors.stream().allMatch(e -> e.phase() == Phase.RENDER),
        "character-set violations are render-phase");
    assertTrue(
        errors.stream().anyMatch(e -> e.message().contains("'c' at position 1")),
        "an error should name the character and its position, got: " + errors);
    assertTrue(
        errors.stream().anyMatch(e -> e.message().contains("BASIC")),
  ...
  void rejectsAValueLongerThanItsElementPermits() {
    ...
    assertTrue(
        errors.stream()
            .anyMatch(e -> e.location().equals("NM1") && e.message().contains("element 1035")),
        "an error should name the element whose bounds were exceeded, got: " + errors);
    assertTrue(
        errors.stream().anyMatch(e -> e.message().contains("61 characters")),
  ...
  void rejectsACodeTheElementDoesNotPermit() throws ValidationException {
    ...
    assertTrue(
        errors.stream()
            .anyMatch(e -> e.location().equals("DMG") && e.message().contains("element 1068")),
  ...
  void aggregatesSpecViolationsWithEachOtherAndWithDelimiterViolations()
    ...
    assertTrue(
        errors.stream().anyMatch(e -> e.message().contains("element separator")),
        "the delimiter violation");
    assertTrue(
        errors.stream().anyMatch(e -> e.message().contains("element 1035")),
        "the length violation");
    assertTrue(
        errors.stream().anyMatch(e -> e.message().contains("element 1068")), "the code violation");
  }
}
