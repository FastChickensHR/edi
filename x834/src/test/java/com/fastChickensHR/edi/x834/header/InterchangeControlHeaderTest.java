...
/**
 * Test class for InterchangeControlHeader. Focuses only on testing the X834-specific defaults and
 * behavior, as general ISA behavior is already tested in ISASegmentTest.
 */
class InterchangeControlHeaderTest {
  ...
  void testDefaultValues() throws ValidationException {
    ...
    InterchangeControlHeader header =
        new InterchangeControlHeader.Builder(context)
            .setInterchangeControlNumber(interchangeControlNumber)
            .build();

    assertEquals(
        InterchangeControlHeader.DEFAULT_AUTHORIZATION_INFO_QUALIFIER,
        header.getAuthorizationInformationQualifier());
    assertEquals(
        InterchangeControlHeader.DEFAULT_AUTHORIZATION_INFO, header.getAuthorizationInformation());
    assertEquals(
        InterchangeControlHeader.DEFAULT_SECURITY_INFO_QUALIFIER,
        header.getSecurityInformationQualifier());
    assertEquals(InterchangeControlHeader.DEFAULT_SECURITY_INFO, header.getSecurityInformation());
    assertEquals(
        InterchangeControlHeader.DEFAULT_INTERCHANGE_SENDER_QUALIFIER,
        header.getInterchangeSenderQualifier());
    assertEquals(
        InterchangeControlHeader.DEFAULT_INTERCHANGE_RECEIVER_QUALIFIER,
        header.getInterchangeReceiverQualifier());
    assertEquals(
        InterchangeControlHeader.DEFAULT_REPETITION_SEPARATOR,
        header.getInterchangeControlStandardsIdentifier());
    assertEquals(
        InterchangeControlHeader.DEFAULT_INTERCHANGE_CONTROL_VERSION,
        header.getInterchangeControlVersionNumber());
    assertEquals(
        InterchangeControlHeader.DEFAULT_ACKNOWLEDGMENT_REQUESTED,
        header.getAcknowledgmentRequested());
    assertEquals(InterchangeControlHeader.DEFAULT_USAGE_INDICATOR, header.getUsageIndicator());
    assertEquals(
        InterchangeControlHeader.DEFAULT_COMPONENT_ELEMENT_SEPARATOR,
        header.getComponentElementSeparator());
  ...
  /**
   * Render golden for the ISA segment assembled from context defaults. Whole-string equality pins
   * the defaults ({@code 00}/{@code 00}/{@code 30}/{@code ZZ}/{@code ^}/{@code 00501}/{@code
   * 0}/{@code T}/{@code :}), both the sender ID (ISA06) and receiver ID (ISA08) right-padded to 15
   * characters, and the ISA09 interchange date as the ISA-native 6-digit {@code YYMMDD} — distinct
   * from the 8-digit {@code CCYYMMDD} that GS04/BGN03/DTP carry — the fixed-width layout the getter
   * assertions never render.
   */
  @Test
