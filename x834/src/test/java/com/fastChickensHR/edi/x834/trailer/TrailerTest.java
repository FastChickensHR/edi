...
class TrailerTest {
  ...
  @BeforeEach
  void setUp() {
    context = new X834Context().setInterchangeControlNumber("000000001").setGroupControlNumber("1");
  }
  ...
  void testBuilderParameterSetting() throws ValidationException {
    ...
    assertEquals(
        transactionSetControlNumber, trailer.getTransactionSetTrailer().getSetControlNumber());
    assertEquals(
        numberOfIncludedSegments, trailer.getTransactionSetTrailer().getTransactionSegmentCount());

    assertEquals(
        numberOfTransactionSetsIncluded,
        trailer.getFunctionalGroupTrailer().getNumberOfTransactionSets());
    assertEquals(groupControlNumber, trailer.getFunctionalGroupTrailer().getGroupControlNumber());

    assertEquals(
        numberOfIncludedFunctionalGroups,
        trailer.getInterchangeControlTrailer().getNumberOfIncludedGroups());
    assertEquals(
        interchangeControlNumber,
        trailer.getInterchangeControlTrailer().getInterchangeControlNumber());
  }
