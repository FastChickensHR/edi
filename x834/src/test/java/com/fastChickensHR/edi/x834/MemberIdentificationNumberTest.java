...
class MemberIdentificationNumberTest {
  ...
  public void testGetSegmentIdentifierReturnsExpectedValue() throws ValidationException {
    MemberIdentificationNumber segment =
        MemberIdentificationNumber.builder().setRef02(referenceIdentification).build();
    segment.setContext(context);

    assertEquals(
        "REF", segment.getSegmentIdentifier(), "Expected segment identifier should be 'REF'");
    assertEquals(
        "REF*DX*MEM987654321~", segment.render().trim(), "The segment is not formatted correctly.");
  }
  ...
  public void testValidationRejectsEmptyRef02() {
    ValidationException exception =
        assertThrows(
            ValidationException.class,
            () -> {
              MemberIdentificationNumber.builder().setRef02("").build();
            },
