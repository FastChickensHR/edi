...
class MemberDemographicsTest {
  ...
  void testBuildWithoutFormatQualifierShouldFail() {
    ValidationException exception =
        assertThrows(
            ...
            () -> MemberDemographics.builder().setBirthDate("19850315").build());

    assertTrue(
        exception.getMessage().contains("DMG01")
  ...
  void testBuildWithoutBirthDateShouldFail() {
    ValidationException exception =
        assertThrows(
            ValidationException.class,
            () -> MemberDemographics.builder().setDateTimePeriodFormatQualifier("D8").build());

    assertTrue(
        exception.getMessage().contains("DMG02") || exception.getMessage().contains("Birth Date"));
  }
