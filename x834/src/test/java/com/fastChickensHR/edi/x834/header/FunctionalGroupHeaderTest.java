...
class FunctionalGroupHeaderTest {
  ...
  /**
   * Tests that the FunctionalGroupHeader uses the correct default values and applies context values
   * correctly.
   */
  @Test
  void testDefaultValuesFromBuilder() throws ValidationException {
    ...
    assertEquals(
        "BE",
        header.getFunctionalIdentifierCode().getCode(),
        "Should use the default functional ID code (BE)");
    assertEquals(
        ResponsibleAgencyCode.ASC_X12,
        header.getResponsibleAgencyCode(),
        "Should use the default responsible agency code (X)");
    assertEquals(
        "005010X220A1",
        header.getVersionReleaseIndustryCode().getCode(),
        "Should use the default version code (005010X220A1)");

    assertEquals(
        senderID, header.getApplicationSenderCode(), "Should use the sender ID from context");
    assertEquals(
        receiverID, header.getApplicationReceiverCode(), "Should use the receiver ID from context");
    assertEquals(
        formattedDate,
        header.getTransactionSetCreationDate(),
        "Should use the formatted date from context");
    assertEquals(
        formattedTime,
        header.getTransactionSetCreationTime(),
        "Should use the formatted time from context");
  }

  /**
   * GS02/GS03 are the <em>application</em> party codes, independent of the ISA06/ISA08 interchange
   * identifiers: a partner may assign a sender code distinct from the mailbox ID and mandate a GS03
   * that is not the interchange receiver ID.
   */
  @Test
  void applicationPartyCodesOverrideTheInterchangeIdentifiers() throws ValidationException {
    context.setApplicationSenderCode("SNDRCODE9").setApplicationReceiverCode("RBG005010X220A1");

    FunctionalGroupHeader header = new FunctionalGroupHeader.Builder(context).build();

    assertEquals(
        "SNDRCODE9",
        header.getApplicationSenderCode(),
        "GS02 should be the application sender code, not the ISA06 sender ID");
    assertEquals(
        "RBG005010X220A1",
        header.getApplicationReceiverCode(),
  ...
  void blankApplicationCodesFallBackToTheInterchangeIdentifiers() throws ValidationException {
    context.setApplicationSenderCode("").setApplicationReceiverCode("   ");
  ...
  void testGetSegmentIdentifierReturnsExpectedValue() throws ValidationException {
    FunctionalGroupHeader header = new FunctionalGroupHeader.Builder(context).build();
    assertEquals("GS", header.getSegmentIdentifier(), "Segment identifier should always be 'GS'");
  }
  ...
  void rendersGsSegmentFromContextDefaults() throws ValidationException {
    ...
    assertEquals(
        "GS*BE*SENDER123*RECEIVER456*20231115*1230*123456789*X*005010X220A1~\n", header.render());
  }
  ...
  void testOverridingDefaultValues() throws ValidationException {
    ...
    assertEquals(
        customFunctionalId,
        header.getFunctionalIdentifierCode(),
        "Should use the custom functional ID code");
    assertEquals(
        customResponsibleAgency,
        header.getResponsibleAgencyCode().getCode(),
    ...
    assertEquals(
        senderID, header.getApplicationSenderCode(), "Should still use the sender ID from context");
    assertEquals(
        receiverID,
        header.getApplicationReceiverCode(),
