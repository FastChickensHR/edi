...
/**
 * A health coverage the member carries — one occurrence of Loop 2300 in the X12 834 (005010X220A1),
 * rendered as an {@code HD} segment followed by its benefit-period dates ({@code DTP*348}/{@code
 * DTP*349}).
 *
 * <p>{@link #maintenanceTypeCode} (HD01) and {@link #insuranceLineCode} (HD03) are required by the
 * segment; a coverage missing either fails generation. The code dictionaries {@link
 * com.fastChickensHR.edi.x834.loop2000.data.MaintenanceTypeCode}, {@link
 * com.fastChickensHR.edi.x834.loop2000.data.InsuranceLineCode} and {@link
 * com.fastChickensHR.edi.x834.loop2000.data.CoverageLevelCode} publish the valid values ({@code
 * fromString(...).getCode()}).
 *
