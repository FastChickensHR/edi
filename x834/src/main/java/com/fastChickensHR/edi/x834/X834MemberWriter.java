...
/**
 * Serializes {@link Member} (and its {@link DependentMember}s) into the sequence of X12 834
 * segments that make up Loop 2000.
 *
 * <p>The X834-specific concerns (delimiters, date formatting, default benefit status, segment
 * ordering, etc.) live here rather than on the domain objects themselves.
 */
class X834MemberWriter {
  ...
  /**
   * Generates all 834 segments for the given member and, recursively, its dependents — in the order
   * required by the 834 specification.
   *
  ...
  private void appendMemberSegments(List<Segment> segments, BaseMember member)
      throws ValidationException {
    INSSegment.Builder ins =
        new INSSegment.Builder()
    ...
    if (policyNumber != null && !policyNumber.isEmpty()) {
      segments.add(
          new MemberPolicyNumber.Builder().setReferenceIdentification(policyNumber).build());
    }

    String memberId = member.getMemberId();
    if (memberId != null && !memberId.isEmpty()) {
      segments.add(
          new MemberIdentificationNumber.Builder()
    ...
    if (subscriberNumber != null && !subscriberNumber.isEmpty()) {
      segments.add(
          new SubscriberNumber.Builder().setReferenceIdentification(subscriberNumber).build());
    }
  ...
  /**
   * Loop 2700/2710/2750 (member reporting categories): an {@code LS*2700} … {@code LE*2700} block
   * wrapping one {@code LX}/{@code N1*75}/{@code REF}(/{@code DTP}) occurrence per {@link
   * ReportingCategory}. The whole block is suppressed when the member carries none, and the {@code
   * LX} assigned number is counted over the emitted occurrences (1-based).
   */
  private void appendReportingCategories(List<Segment> segments, BaseMember member)
    ...
    for (ReportingCategory category : categories) {
      ...
      segments.add(
          MemberReportingCategoryName.builder()
   ...
   *
   * <p>The {@code LX} counter is assigned at render time over the emitted occurrences (1-based),
   ...
   *
   * <p>The 2310 {@code N3}/{@code N4}/{@code PER} segments are not modelled — no profiled carrier
   * asks for a provider's address or phone, only their name and identifier.
   */
  private void appendProviders(List<Segment> segments, BaseMember member)
      throws ValidationException {
    ...
    if (providers.size() > Provider.MAX_PER_MEMBER) {
      throw new ValidationException(
          "A member carries at most "
              + Provider.MAX_PER_MEMBER
              + " provider loops (2310); got "
              + providers.size());
    }
    int assignedNumber = 1;
      ...
      segments.add(
          ProviderName.builder()
              ...
              .setProviderIdentification(
                  provider.getIdentifierQualifier(), emptyToNull(provider.getIdentifier()))
              .build());
   ...
   */
  private void appendProviderChange(List<Segment> segments, Provider provider)
      throws ValidationException {
    ...
    segments.add(
        ProviderChange.builder()
            ...
            .setDate(DateFormat.D8.format(provider.getChangeDate()))
   ...
   * followed by its optional group-number {@code REF}, its coordination {@code DTP*344}/ {@code
   * DTP*345} dates, and the 2330 {@code NM1} naming that plan. Emitted after the 2300 coverage
   * segments and before the 2700 block, and suppressed entirely when the member has no other
   * coverage.
   *
   * <p>The 834 permits five occurrences. A member carrying more is rejected rather than truncated —
   * dropping someone's other insurance would produce a file that reads as though they simply do not
   * have it.
   */
  private void appendCoordinationOfBenefits(List<Segment> segments, BaseMember member)
    ...
    if (others.size() > CoordinationOfBenefits.MAX_PER_MEMBER) {
      throw new ValidationException(
          "A member carries at most "
              + CoordinationOfBenefits.MAX_PER_MEMBER
              + " coordination-of-benefits loops (2320); got "
              + others.size());
    }
    for (CoordinationOfBenefits other : others) {
      ...
      addQualifiedDate(
          segments, DateTimeQualifier.COORDINATION_OF_BENEFITS_BEGIN, other.getBeginDate());
      addQualifiedDate(
          segments, DateTimeQualifier.COORDINATION_OF_BENEFITS_END, other.getEndDate());
      if (!isBlank(other.getRelatedEntityName())) {
        segments.add(
            CoordinationOfBenefitsRelatedEntityName.builder()
  ...
  private void addQualifiedDate(
      List<Segment> segments, DateTimeQualifier qualifier, LocalDateTime date)
      throws ValidationException {
    if (date == null) {
      return;
    }
  ...
  private void appendMemberName(List<Segment> segments, BaseMember member)
      throws ValidationException {
    if (isBlank(member.getLastName())) {
      return;
    }
    MemberName.Builder name =
        MemberName.builder()
   ...
   *
   * <p>The member's explicit {@link MemberCommunication}s come first, in the order they were added.
   * The {@code phoneNumber} and {@code email} conveniences then contribute {@code HP} and {@code
   * EM} — but only when no explicit communication already claims that qualifier, so a member given
   * both a {@code phoneNumber} and an explicit work number does not emit the same channel twice.
   * Precedence is the explicit one's: it says which qualifier the caller meant.
   *
   * <p>A PER carries at most three pairs, and a member with more is rejected by the builder rather
   * than truncated — losing a member's fourth contact number silently is the class of bug this
   * whole segment exists to end.
   */
  private void appendCommunications(List<Segment> segments, BaseMember member)
      throws ValidationException {
    List<MemberCommunication> channels = new ArrayList<>();
    for (MemberCommunication communication : member.getCommunications()) {
      if (communication != null
          && communication.getQualifier() != null
  ...
  /**
   * Adds a {@code phoneNumber}/{@code email} convenience unless an explicit channel claims its
   * qualifier.
   */
  private static void addConvenience(
      List<MemberCommunication> channels, CommunicationNumberQualifier qualifier, String number) {
    if (isBlank(number)) {
      return;
    }
    for (MemberCommunication channel : channels) {
   ...
   */
  private void appendResidenceAddress(List<Segment> segments, BaseMember member)
      throws ValidationException {
    if (!isBlank(member.getAddressLine1())) {
      segments.add(
          MemberResidenceStreetAddress.builder()
              .setAddressLine1(member.getAddressLine1())
              .setAddressLine2(emptyToNull(member.getAddressLine2()))
              .build());
    }
    if (!isBlank(member.getCity())
        && !isBlank(member.getState())
        && !isBlank(member.getZipCode())) {
      segments.add(
   ...
   */
  private void appendMailingAddress(List<Segment> segments, BaseMember member)
      throws ValidationException {
    ...
    if (mailing.hasCityStateZip()) {
      segments.add(
          MemberMailingCityStateZipCode.builder()
   ...
   * normal case.
   *
   * <p>ICM01 (frequency) and ICM02 (amount) are mandatory, so an income carrying only the later
   * elements is rejected rather than emitted with empty mandatory slots. This is the case a carrier
   * hits when it wants just the ICM04 location identifier — BCBS Kansas puts its department number
   * there — and it is a property of the 834 rather than a rule invented here.
   */
   ...
   * BCBSM notes the health-related code "may be required for specific employer groups", so whether
   * to send it is a config-time answer and absence is the normal case.
   */
  private void appendHealthInformation(List<Segment> segments, BaseMember member)
      throws ValidationException {
  ...
  /**
   * Loop 2100A LUI (member language), emitted after the HLH — one per language the member uses, in
   * the order added. Suppressed when the member carries none.
   */
  private void appendLanguages(List<Segment> segments, BaseMember member)
      throws ValidationException {
  ...
  /**
   * Loop 2200 (disability): per disability, a {@code DSB} followed by its {@code DTP*360}/ {@code
   * DTP*361} period dates. Emitted after the 2100 loops and before the 2300 coverage segments, and
   * suppressed when the member has none.
   *
   * <p>BCBSM asks for the period rather than the detail, but DSB01 is mandatory, so the dates
   * cannot travel alone — a sponsor sending a disability period must also say what kind of
   * disability it is. The guide itself is wrong on this point, reading "DTP01 … Start / DTP02 …
   * End" when DTP02 is the date-format qualifier; two dates are two DTP segments, which is what
   * this emits.
   */
  /**
   * Loop 2300 (health coverage): per coverage, an {@code HD} followed by its benefit-period {@code
   * DTP*348}/{@code DTP*349} dates. Emitted after the member's trailing segments and before the
   * 2310 block, and suppressed when the member has none. HD01 and HD03 are required by the segment,
   * so a coverage missing either fails generation.
   */
  private void appendHealthCoverages(List<Segment> segments, BaseMember member)
      throws ValidationException {
    for (HealthCoverage coverage : member.getHealthCoverages()) {
      segments.add(
          new HealthCoverageSegment(
  ...
  private void appendDisabilities(List<Segment> segments, BaseMember member)
      throws ValidationException {
    for (Disability disability : member.getDisabilities()) {
      segments.add(
          MemberDisability.builder()
              ...
              .build());
      addQualifiedDate(
          segments, DateTimeQualifier.INITIAL_DISABILITY_PERIOD_START, disability.getStartDate());
      addQualifiedDate(
          segments, DateTimeQualifier.INITIAL_DISABILITY_PERIOD_END, disability.getEndDate());
    }
  }

  /** Loop 2100A DMG (member demographics). Emitted when a birth date is present. */
  private void appendDemographics(List<Segment> segments, BaseMember member)
      throws ValidationException {
    if (member.getBirthDate() == null) {
      return;
    }
    segments.add(
        new MemberDemographics.Builder()
            .setDateTimePeriodFormatQualifier(DateFormat.D8.getFormat())
            .setBirthDate(DateFormat.D8.format(member.getBirthDate()))
  ...
  private void addDateSegment(
      List<Segment> segments, MemberDateQualifier qualifier, LocalDateTime date)
      throws ValidationException {
    if (date == null) {
      return;
    }
