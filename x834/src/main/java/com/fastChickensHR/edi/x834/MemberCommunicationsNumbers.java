...
/**
 * The PER (Member Communications Numbers) segment in Loop 2100A of the X12 834 (005010X220A1).
 *
 * <p>PER01 is fixed to {@link #INSURED_PARTY} — the member named by the 2100A {@code NM1} is the
 * contact — and PER02 (contact name) is left absent, since that member is already named by the
 * preceding {@code NM1}. What varies is the up-to-three qualifier/number pairs, added with {@link
 * PERSegment.AbstractBuilder#addCommunicationNumber}. A carrier requiring a home phone and an email
 * therefore renders as {@code PER*IP**HP*5551234567*EM*a@b.com~}.
 */
@Getter
class MemberCommunicationsNumbers extends PERSegment {

  /**
   * PER01 contact function code {@code IP} — Insured Party. The 834 names the member itself as the
   * contact for its own communication numbers.
   */
