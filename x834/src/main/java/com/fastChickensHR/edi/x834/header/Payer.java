 ...
 *
 * <p>Produces the N1 segment that identifies the payer (insurer) in the header portion of the 834,
 * following the plan sponsor (Loop 1000A). Extends the generic {@link N1Segment}.
 */
class Payer extends N1Segment {
  ...
  /**
   * Default N103 identification code qualifier {@code "FI"} (Federal Taxpayer's Identification
   * Number).
   */
   ...
   *
   * <p>The identification qualifier (N103={@value #DEFAULT_IDENTIFICATION_CODE_QUALIFIER}) is
   * <em>not</em> pre-seeded: under X12 rule P0304 the qualifier and the identifier (N104) must
   * travel together, so it is set only alongside a payer identifier (see {@code
   * Header.createDefaultPayerBuilder}). A name-only payer therefore emits {@code N1*IN*<name>} with
   * no dangling qualifier.
   */
