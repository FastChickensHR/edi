 ...
 *
 * <p>The BGN segment defines the beginning of a transaction set and provides transaction purpose,
 * reference identification, and date/time information.
 *
 * <h3>X12 834 Reference</h3>
 *
 * <ul>
 *   <li>BGN01 — Transaction Set Purpose Code (M, ID 2/2)
 *   <li>BGN02 — Reference Identification (M, AN 1/50)
 *   <li>BGN03 — Date, CCYYMMDD (M, DT 8/8)
 *   <li>BGN04 — Time, HHMM/HHMMSS/HHMMSSDD (O, TM 4/8)
 *   <li>BGN05 — Time Zone Code (O, ID 2/2)
 *   <li>BGN06 — Original Reference Number (O, AN 1/50)
 *   <li>BGN07 — Transaction Type Code (O, ID 2/2)
 *   <li>BGN08 — Action Code (O, ID 1/2)
 *   <li>BGN09 — Security Level Code (O, ID 2/2)
 * </ul>
 *
 * <h3>Default Handling</h3>
 *
 * <p>Segment-specific defaults are exposed as {@code DEFAULT_*} constants. Concrete subclasses
 * (e.g. {@code BeginningSegment} in the x834 header) are responsible for applying defaults to
 * optional fields where appropriate.
 */
@Getter
abstract class BGNSegment extends Segment {
  ...
  /**
   * Constructs a new BGN segment from the supplied builder and performs field-level validation.
   *
  ...
  /**
   * Performs field-level validation of required and optional BGN elements, including length and
   * format constraints.
   *
  ...
  private void validateRequiredFields() throws ValidationException {
    ...
    if (bgn02.length() > BGN02_MAX_LENGTH) {
      throw new ValidationException(
          "Reference Identification (BGN02) must be " + BGN02_MAX_LENGTH + " characters or less");
    }
    ...
    if (bgn06 != null && bgn06.length() > BGN06_MAX_LENGTH) {
      throw new ValidationException(
          "Original Reference Number (BGN06) must be " + BGN06_MAX_LENGTH + " characters or less");
    }
  }
  ...
  public String[] getElementValues() {
    return new String[] {
      ...
      bgn03,
      bgn04,
      bgn05 != null ? bgn05.getCode() : null,
      bgn06,
      bgn07 != null ? bgn07.getCode() : null,
      bgn08 != null ? bgn08.getCode() : null,
      bgn09 != null ? bgn09.getCode() : null
    };
  }
  ...
  /**
   * Abstract builder for BGN segments. Concrete subclasses are free to apply segment-specific
   * defaults (e.g. {@link #DEFAULT_TRANSACTION_SET_PURPOSE_CODE}) inside their constructors prior
   * to {@link #build()} being called.
   *
