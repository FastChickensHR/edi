...
/**
 * The two character sets X12 names — the vocabulary a trading partner picks from when it restricts
 * what an interchange's data may contain (X12.6 §3.3.2; HIPAA 005010 TR3 Appendix
 * B.1.1.2.2–B.1.1.2.3).
 *
 * <p>{@link #BASIC} is upper-case letters, digits, space and a short list of punctuation. {@link
 * #EXTENDED} adds lower-case letters, {@code @}, and further punctuation. A partner that says
 * "basic set, plus lower case and {@code @}" is naming {@code BASIC} plus two members of {@code
 * EXTENDED}: consumers seed the named set plus an explicit extra-character list and check each
 ...
 * delimiter may never occur in a data value (x12.org RFI 2611) — and {@code *} and {@code :}, the
 * usual element and component separators, are members of {@code BASIC}. Enforcement therefore pairs
 * a character-set check with a delimiter check against the interchange's own separators, rather
 * than subtracting one interchange's delimiter choice from these published sets.
 *
 * <p><strong>Scope of {@code EXTENDED}.</strong> Its members are the 005010 TR3 Table B.2 list.
 * That appendix is explicitly a partial transcription of X12.6 §3.3.2, which also admits select
 * language-specific characters (e.g. {@code ü}); those stay out until a trading partner demands
 * them, so every published set is ASCII. Two members — {@code ^} and {@code `} — are transcribed
