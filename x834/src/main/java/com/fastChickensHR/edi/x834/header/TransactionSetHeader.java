 ...
 * (005010X220A1), sitting just inside the GS/GE functional group. Includes key fields such as the
 * transaction set identifier code (ST01), transaction set control number (ST02), and implementation
 * convention reference (ST03). Extends the generic ST segment.
 */
@Getter
class TransactionSetHeader extends STSegment {
  /**
   * Default ST01 transaction set identifier code {@code "834"} (Benefit Enrollment and
   * Maintenance).
   */
  private static final String DEFAULT_TRANSACTION_SET_ID = "834";

  /** Default ST02 transaction set control number {@code "0001"}. */
  private static final String DEFAULT_CONTROL_NUMBER = "0001";

  /** Default ST03 implementation convention reference {@code "005010X220A1"} (the 834 guide). */
  private static final String DEFAULT_CONVENTION_REFERENCE = "005010X220A1";
  ...
  public static class Builder extends STSegment.AbstractBuilder<TransactionSetHeader.Builder> {
    ...
    /**
     * Creates a builder using the context's transaction set control number for ST02, with the
     * default ST01 (834) and ST03 (005010X220A1) values.
     *
