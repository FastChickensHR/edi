 ...
 *
 * <p>NM101 defaults to {@link EntityIdentifierCode#PROVIDER} ({@code 1P}) and NM102 to {@link
 * #PERSON_ENTITY_TYPE} ({@code 1}), a physician being a person. NM103–NM105 carry the provider's
 * name and NM108/NM109 identify them.
 *
 * <p><b>Which identifier</b> is a carrier answer, not a default: Florida Blue mandates {@link
 * IdentificationCodeQualifier#CMS_NPI} ({@code XX}) — "Only the above code is valid" — while
 * CareFirst uses {@link IdentificationCodeQualifier#SERVICE_PROVIDER} ({@code SV}), its own legacy
 * ID, "until CareFirst provides the NPI". BCBSM asks for the NPI "when available", falling back to
 * its own physician number. So the qualifier is supplied per provider.
 *
 * <p>NM108 and NM109 are a paired presence, enforced here: an identifier with no qualifier does not
 * say what kind of number it is, and a qualifier with no identifier says nothing at all.
 */
@Getter
class ProviderName extends NM1Segment {
  ...
  /**
   * Builder for {@link ProviderName}; NM101 defaults to {@code 1P} (Provider) and NM102 to {@link
   * #PERSON_ENTITY_TYPE}.
   */
  public static class Builder extends NM1Segment.AbstractBuilder<Builder> {
    ...
    public Builder setProviderIdentification(
        IdentificationCodeQualifier qualifier, String identifier) {
