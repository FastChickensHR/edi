...
/**
 * Represents the ST (Transaction Set Header) segment in the X12 834 (005010X220A1) format.
 *
 * <p>This segment opens a transaction set, naming the transaction type, a control number, and the
 * implementation convention it conforms to. It is closed by a matching {@code SESegment}.
 *
 * <p>Element/position map:
 *
 * <ul>
 *   <li>ST01 = transaction set identifier code (e.g. "834")
 *   <li>ST02 = transaction set control number (4–9 characters; matched by SE02)
 *   <li>ST03 = implementation convention reference (optional, max 35 characters)
 * </ul>
 */
@Getter
abstract class STSegment extends Segment {
  ...
  private void validateRequiredFields() throws ValidationException {
    ...
    if (st02.length() < 4 || st02.length() >= 10) {
      throw new ValidationException(
          "ST02 (Transaction Set Control Number) must be between 4 and 9 characters");
    }
    ...
    if (st03 != null && st03.length() > 35) {
      throw new ValidationException(
          "ST03 (Implementation Convention Reference) must be 35 characters or less");
    }
  }
