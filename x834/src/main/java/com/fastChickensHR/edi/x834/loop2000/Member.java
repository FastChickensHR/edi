...
/**
 * Represents a member (employee/participant/person). May be a primary member (subscriber) carrying
 * zero or more dependents.
 *
 * <p>This is a pure domain object — it carries no knowledge of the X12 834 wire format. Translation
 * to 834 segments is performed by {@code X834MemberWriter}.
 */
@Getter
@Setter
public class Member extends BaseMember {
  ...
  /**
   * Validates this member has the minimum required fields: the INS-segment trio (member indicator,
   * relationship code, maintenance type code) that {@code X834MemberWriter} dereferences
   * unconditionally, plus each dependent's own validation.
   *
