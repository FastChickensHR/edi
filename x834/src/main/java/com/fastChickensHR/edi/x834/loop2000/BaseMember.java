 ...
 *
 * <p>This is a plain data object describing a person. It is intentionally decoupled from any
 * specific EDI transaction format; serialization to the X12 834 wire format is the responsibility
 * of a dedicated writer (see {@code X834MemberWriter}).
 */
@Getter
@Setter
public abstract class BaseMember {
  ...
  /**
   * NM108 — identification code qualifier carried in the member name segment (Loop 2100A NM1), e.g.
   * {@code 34} for a Social Security Number. Must be paired with {@link #nameId}.
   */
  ...
  /**
   * INS04 — why this maintenance is happening (element 1203), e.g. termination of benefits.
   * Optional: the 834 requires a maintenance <em>type</em> on every member, never a reason.
   */
  ...
  /**
   * The member's telephone number, emitted in the Loop 2100A {@code PER} under {@link
   * CommunicationNumberQualifier#HOME_PHONE} — the qualifier carriers most often ask for (Anthem's
   * worked examples send {@code PER*IP**HP*<phone>}).
   *
   * <p>A convenience for the common case. To send the number under a different qualifier — work,
   * cellular, alternate — add a {@link MemberCommunication} instead; an explicit communication for
   * a qualifier takes precedence over this field.
   */
  protected String phoneNumber;

  /**
   * The member's email address, emitted in the Loop 2100A {@code PER} under {@link
   * CommunicationNumberQualifier#ELECTRONIC_MAIL}.
   *
  ...
  /**
   * Adds a typed address to this member. Multiple types may coexist; adding a second address of a
   * type that already exists replaces the previous one so a member has at most one of each kind.
   *
  ...
  private Address residenceOrCreate() {
    return getAddress(AddressType.RESIDENCE)
        .orElseGet(
            () -> {
  ...
  /**
   * Trailing segments that belong to <em>this</em> member — notably custom REF extensions. They are
   * emitted by {@code X834MemberWriter} at the end of this member's own segment stream (just before
   * the {@link #addHealthCoverage(HealthCoverage) 2300 coverage block}), so they stay nested inside
   * the member's own loop rather than being batched after every member.
   */
  private final List<Segment> additionalSegments = new ArrayList<>();

  /**
   * Appends a trailing segment to this member — e.g. a custom REF extension. Order is preserved.
   * Health coverage is not a segment here: it is modeled by {@link
   * #addHealthCoverage(HealthCoverage)} and rendered by the writer.
   *
  ...
  /**
   * This member's reporting categories (Loop 2700/2710/2750). Emitted by {@code X834MemberWriter}
   * as an {@code LS*2700} … {@code LE*2700} block after the member's 2300 segments, one {@code
   * LX}/{@code N1*75}/{@code REF} occurrence per entry. Empty for a member that carries none, in
   * which case no block is emitted.
   */
  private final List<ReportingCategory> reportingCategories = new ArrayList<>();

  /**
   * This member's communication numbers (Loop 2100A {@code PER}). Emitted by {@code
   * X834MemberWriter} as a single {@code PER} carrying one qualifier/number pair per entry, after
   * the member's {@code NM1}. Empty for a member that carries none, in which case no {@code PER} is
   * emitted.
   *
   ...
   *
   * <p>Order is preserved and determines which element pair each occupies — the first added becomes
   * PER03/04, then PER05/06, then PER07/08. An explicit communication takes precedence over the
   * {@link #phoneNumber}/{@link #email} conveniences when both name the same qualifier. The 834
   * permits at most {@value
   * com.fastChickensHR.edi.x834.loop2000.loop2100A.MemberCommunication#MAX_PER_MEMBER} per member;
   * a member carrying more is rejected when written, not silently truncated.
   *
  ...
  /**
   * Adds a way to reach this member — convenience for {@link
   * #addCommunication(MemberCommunication)}.
   *
  ...
  /**
   * The health coverages this member carries (Loop 2300). Emitted by {@code X834MemberWriter} after
   * the member's trailing segments ({@link #addSegment(Segment)}) and before the 2310 block, one
   * {@code HD}(/{@code DTP*348}/{@code DTP*349}) block per entry. Empty for a member with none, in
   ...
   *
   * <p>Order is preserved, and the {@code LX} assigned number is counted over the emitted
   * occurrences. The 834 permits at most {@value
   * com.fastChickensHR.edi.x834.loop2000.loop2310.Provider#MAX_PER_MEMBER} per member; a member
   * carrying more is rejected when written, not silently truncated.
   *
   ...
   * after the member's 2300 coverage segments, one {@code COB}(/{@code REF}/{@code DTP}/2330 {@code
   * NM1}) block per entry. Empty for a member with no other coverage, in which case nothing is
   * emitted.
   *
   ...
   *
   * <p>Order is preserved. The 834 permits at most {@value
   * com.fastChickensHR.edi.x834.loop2000.loop2320.CoordinationOfBenefits#MAX_PER_MEMBER}
  ...
  /**
   * Adds a reporting category (one Loop 2710/2750 occurrence) to this member. Order is preserved;
   * the {@code LX} assigned number is set at render time over the emitted occurrences.
   *
