...
/**
 * A single member reporting category — one occurrence of Loop 2710/2750 in the X12 834
 * (005010X220A1).
 *
 * <p>A reporting category is a name/value pair the carrier collects against a member: the {@link
 * #name} labels what is being reported (carried in the 2750 {@code N1*75} segment) and the {@link
 * #value} is what is reported for it (carried in the following {@code REF} segment under {@link
 * #referenceQualifier}, which defaults to {@code ZZ}, Mutually Defined). An occurrence may also
 * carry a {@link #date} (emitted as a {@code DTP} segment) when {@link #dateQualifier} is set to
 * state what the date means.
 *
...
public class ReportingCategory {
  ...
  /**
   * The qualifier stating what {@link #date} means (2750 {@code DTP} DTP01); required when a date
   * is present.
   */
