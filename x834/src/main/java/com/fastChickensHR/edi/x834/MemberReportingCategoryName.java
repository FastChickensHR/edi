...
/**
 * The N1 (Party Identification) segment that names a member reporting category in Loop 2750 of the
 * X12 834 (005010X220A1).
 *
 * <p>N101 is fixed to {@code 75} (Participant); N102 carries the reporting-category <em>name</em> —
 * the label the carrier uses to identify what this occurrence reports (e.g. {@code
 * INDIVIDUALREPNAME}, {@code RELATIONSHIP}). The category's value rides the REF segment that
 * follows this one in the same 2750 occurrence.
 *
 * <p>The name is a carrier-specific label, so it is supplied per occurrence rather than drawn from
 * a fixed enum.
 */
