...
/**
 * The outcome of {@link ElementSpec#permits(java.util.Collection)}: whether a proposed set of codes is a
 * subset of what the standard permits at one position, and if not, exactly which codes are not members.
 *
 * <p>The unknown codes are reported in the order they were proposed, so a caller narrowing a code list
 * (a trading-partner restriction, say) can point at the offending entries in the order it holds them.
 */
