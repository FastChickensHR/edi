...
/**
 * Represents the DTP (Date or Time or Period) segment in the X12 834 (005010X220A1) Benefit
 * Enrollment and Maintenance transaction.
 *
 * <p>This segment expresses a date, time, or period together with a qualifier that states what the
 * date means and a format code that states how it is encoded.
 *
 * <p>Element/position map:
 *
 * <ul>
 *   <li>DTP01 = date/time qualifier — what the date represents (required)
 *   <li>DTP02 = date/time format qualifier — how DTP03 is formatted (required)
 *   <li>DTP03 = date/time period value, max 35 characters (required)
 * </ul>
 */
@Getter
abstract class DTPSegment extends Segment {
  ...
  public abstract static class AbstractBuilder<T extends AbstractBuilder<T>> {
    ...
    public T setDateTimePeriod(LocalDateTime value, DateFormat format) {
      this.dtp03 = format.format(value);
      return self();
    }
