...
/**
 * Represents the Functional Group Header (GS) segment specific to the X834 format (005010X220A1).
 * The GS opens the functional group inside the ISA/IEA interchange envelope and precedes the
 * transaction set header (ST). Extends the generic GS segment with X834-specific defaults.
 */
@Getter
class FunctionalGroupHeader extends GSSegment {
   ...
   */
  private static final FunctionalIdentifierCode DEFAULT_FUNCTIONAL_ID_CODE =
      FunctionalIdentifierCode.fromString("BE");

  /** Default GS07 responsible agency code (ASC X12). */
  private static final ResponsibleAgencyCode DEFAULT_RESPONSIBLE_AGENCY_CODE =
  ...
  public static class Builder extends GSSegment.AbstractBuilder<Builder> {
     ...
     * ({@link X834Context#getSenderID() senderID} / {@link X834Context#getReceiverID() receiverID})
     * when unset. GS mirroring ISA is the common case, so callers that never set the application
     * codes emit exactly what they did before.
     *
