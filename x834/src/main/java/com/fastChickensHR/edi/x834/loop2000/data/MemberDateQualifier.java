...
/**
 * Represents Member Level Date Qualifiers, which are a subset of DateTimeQualifier. This enum maps
 * the member-specific qualifiers to the appropriate DateTimeQualifier codes.
 */
@Getter
public enum MemberDateQualifier implements EdiCodeEnum {
   ...
   * {@link #TERMINATION}), so a bare code resolves to whichever constant registered it last;
   * resolve by enum name or synonym when the distinction matters.
   *
