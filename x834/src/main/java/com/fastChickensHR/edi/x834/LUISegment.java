 ...
 *
 * <p>The segment names a language the member uses, in Loop 2100A. BCBSM's BCN Advantage product
 ...
 *
 * <ul>
 *   <li>LUI01 = identification code qualifier (66) — what kind of language code LUI02 is
 *   <li>LUI02 = identification code (67, AN 2/80) — the language code itself
 *   <li>LUI03 = description (352, AN 1/80) — the language named in words
 * </ul>
 *
 ...
 *
 * <p>Two published relational conditions are enforced. LUI01 and LUI02 require each other — a code
 ...
 */
@Getter
abstract class LUISegment extends Segment {
  ...
  private void validate() throws ValidationException {
    ...
    if (codePresent && lui02.length() < MIN_LANGUAGE_CODE_LENGTH) {
      throw new ValidationException(
          "Identification Code (LUI02) must be at least "
              + MIN_LANGUAGE_CODE_LENGTH
              + " characters; got '"
              + lui02
              + "'");
    }
    if (codePresent && lui02.length() > MAX_LANGUAGE_CODE_LENGTH) {
      throw new ValidationException(
          "Identification Code (LUI02) must be "
              + MAX_LANGUAGE_CODE_LENGTH
              + " characters or less");
    }
    if (descriptionPresent && lui03.length() > MAX_DESCRIPTION_LENGTH) {
      throw new ValidationException(
          "Description (LUI03) must be " + MAX_DESCRIPTION_LENGTH + " characters or less");
    }
  }
  ...
  public abstract static class AbstractBuilder<T extends AbstractBuilder<T>> {
    ...
    /**
     * Sets LUI01/LUI02 — the language code and the scheme it belongs to. Both together, since the
     * 834 requires each if the other is present.
     *
