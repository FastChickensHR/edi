...
/**
 * Represents the PER (Administrative Communications Contact) segment in the X12 834 (005010X220A1)
 * Benefit Enrollment and Maintenance transaction.
 *
 * <p>The segment carries the communication numbers for the party named in the enclosing loop: a
 * contact function code, then up to three qualifier/number pairs. In the 834 it appears in Loop
 * 2100A as the member's communications numbers (see {@code MemberCommunicationsNumbers}).
 *
 * <p>Element/position map (data elements per the 834 TR3):
 *
 * <ul>
 *   <li>PER01 = contact function code (366)
 *   <li>PER02 = contact name (93)
 *   <li>PER03 = communication number qualifier (365)
 *   <li>PER04 = communication number (364)
 *   <li>PER05 = communication number qualifier (365)
 *   <li>PER06 = communication number (364)
 *   <li>PER07 = communication number qualifier (365)
 *   <li>PER08 = communication number (364)
 * </ul>
 *
 * PER09 is Not Used in the 834 and is not modelled.
 *
 * <p>X12 syntax rules P0304, P0506 and P0708 make each qualifier/number a <em>paired presence</em>:
 * within a pair, if either side is present the other is required. A lone qualifier would render as
 * a dangling {@code *HP} tail and a lone number as {@code **555...}, both non-conformant, so both
...
abstract class PERSegment extends Segment {
  ...
  private static void validatePair(
      CommunicationNumberQualifier qualifier,
      String number,
      String rule,
      String qualifierPosition,
    ...
    if (numberPresent && number.length() > MAX_COMMUNICATION_NUMBER_LENGTH) {
      throw new ValidationException(
          "Communication Number ("
              + numberPosition
              + ") must be "
              + MAX_COMMUNICATION_NUMBER_LENGTH
              + " characters or less");
    }
  }
  ...
  public abstract static class AbstractBuilder<T extends AbstractBuilder<T>> {
     ...
     *
     * <p>Callers state <em>which</em> channels a member has, not which element positions they
     * occupy, so the pairs fill in call order. A PER carries at most {@link
     * MemberCommunication#MAX_PER_MEMBER} pairs; a fourth is rejected rather than dropped, because
     ...
     * @throws ValidationException if the segment already carries {@link
     *     MemberCommunication#MAX_PER_MEMBER} pairs
     */
    public T addCommunicationNumber(CommunicationNumberQualifier qualifier, String number)
      ...
      if (per03 == null) {
        ...
        throw new ValidationException(
            "A PER segment carries at most "
                + MemberCommunication.MAX_PER_MEMBER
