...
/**
 * Utility class providing functions for text formatting commonly used in EDI files, particularly
 * for handling fixed-width fields and padding requirements.
 */
final class TextUtils {
  ...
  /**
   * Pads a string to a fixed width with spaces on the right. If the input is longer than the width,
  ...
  public static String padRight(String input, int width) {
    ...
    if (input.length() > width) {
      ...
      LOGGER.warning(
          "Input string '"
              + input
              + "' exceeds specified width of "
              + width
              + ". String will be truncated to: '"
              + substring
              + "'");
      return substring;
    }
  ...
  /**
   * Pads a string to a fixed width with spaces on the left. If the input is longer than the width,
   * it will be truncated from the left and a warning will be logged.
   *
   * @param input The string to pad
   * @param width The desired fixed width
   * @return A string padded to the specified width
