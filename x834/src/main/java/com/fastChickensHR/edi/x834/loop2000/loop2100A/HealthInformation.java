 ...
 *
 * <p>{@link #healthRelatedCode} is what carriers actually ask for: tobacco and substance use, a
 * rating input for individual and small-group products. BCBSM notes it "may be required for
 * specific employer groups", so whether to send it is a config-time answer. The height and weight
 * elements round the segment out.
 *
 * <p>Heights and weights are carried as strings so the caller's own precision reaches the wire
 * unchanged.
 *
