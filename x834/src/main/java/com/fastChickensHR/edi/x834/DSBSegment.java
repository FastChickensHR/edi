 ...
 *
 * <p>The segment opens Loop 2200 and states the member's disability status. BCBSM's interest is the
 ...
 *
 * <ul>
 *   <li>DSB01 = disability type code (1146) — <b>mandatory</b>
 *   <li>DSB02 = quantity (380, R 1/15)
 *   <li>DSB03 = occupation code (1149, ID 4/6)
 *   <li>DSB04 = work intensity code (1154, ID 1/1)
 *   <li>DSB05 = product option code (1161, ID 1/2)
 *   <li>DSB06 = monetary amount (782, R 1/18)
 * </ul>
 *
 ...
 *
 * <p>DSB01 being mandatory means the disability <em>dates</em> cannot travel alone: a sponsor
 * sending a disability period must also say what kind of disability it is. That is the 834's
 * constraint, and this class refuses the alternative rather than emitting a segment with an empty
 * mandatory element.
 */
@Getter
  ...
  private void validate() throws ValidationException {
    ...
    if (dsb02 != null && dsb02.length() > MAX_QUANTITY_LENGTH) {
      throw new ValidationException(
          "Quantity (DSB02) must be " + MAX_QUANTITY_LENGTH + " characters or less");
    }
    if (dsb03 != null
        && !dsb03.isEmpty()
        && (dsb03.length() < MIN_OCCUPATION_CODE_LENGTH
            || dsb03.length() > MAX_OCCUPATION_CODE_LENGTH)) {
      throw new ValidationException(
          "Occupation Code (DSB03) must be between "
              + MIN_OCCUPATION_CODE_LENGTH
              + " and "
              + MAX_OCCUPATION_CODE_LENGTH
              + " characters; got '"
              + dsb03
              + "'");
    }
    if (dsb06 != null && dsb06.length() > MAX_MONETARY_AMOUNT_LENGTH) {
      throw new ValidationException(
          "Monetary Amount (DSB06) must be " + MAX_MONETARY_AMOUNT_LENGTH + " characters or less");
    }
  }
