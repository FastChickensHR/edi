...
/**
 * The 999 (and 997) acknowledgment dialect's {@code Location.name} vocabulary — the tokens {@link
 * X999FileParser} emits so a consumer correlating acknowledgments can read one by location, the
 * mirror of how the 834 generator reads its own {@link com.fastChickensHR.edi.core.Location}s.
 *
 * <p>File-level fields describe the acknowledged <em>functional group</em> (the reply is one group
 * per 999); each record describes one acknowledged <em>transaction set</em>. Values are the raw X12
 * codes — the parser stays dialect-dumb and leaves accept/reject interpretation to the caller.
 */
public final class X999 {
  ...
  public static final String ACKNOWLEDGED_INTERCHANGE_CONTROL_NUMBER =
      "acknowledgedInterchangeControlNumber";
