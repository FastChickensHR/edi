...
/**
 * The X12 999 / 997 implementation of the {@link FileParser} seam: reads a functional
 * acknowledgment into a format-neutral {@link FileContent}. An acknowledgment replies to one
 * functional group, so the file-level fields carry the acknowledged group (functional id, group
 * control number, group status), and each {@link Record} carries one acknowledged transaction set
 * (its control number and status) — see {@link X999}.
 *
 * <p>The parser is dialect-dumb: it walks segments and emits the raw X12 codes as {@link Field}s at
 * their {@link X999} locations, leaving accept/reject interpretation and correlation to the caller.
 * It reads {@code AK1} (group), {@code AK2} (transaction set), {@code AK5}/{@code IK5} (transaction
 * set response) and {@code AK9} (group response); interior error-detail segments ({@code
 * AK3/AK4/IK3/IK4}) are skipped in this pass. Delimiters are taken from the interchange's {@code
 * ISA} envelope, defaulting to {@code *} (element) and {@code ~} (segment).
 */
public final class X999FileParser implements FileParser {
  ...
  public FileContent parse(String raw) {
    ...
    for (String rawSegment :
        raw.split(java.util.regex.Pattern.quote(String.valueOf(segmentTerminator)))) {
      String segment = rawSegment.strip();
      if (segment.isEmpty()) {
        continue;
      }
      String[] e =
          segment.split(java.util.regex.Pattern.quote(String.valueOf(elementSeparator)), -1);
      switch (e[0]) {
        case "ISA" ->
            addField(fileFields, RecordLevel.FILE, X999.INTERCHANGE_CONTROL_NUMBER, at(e, 13));
        case "TA1" -> {
          addField(
              fileFields, RecordLevel.FILE, X999.ACKNOWLEDGED_INTERCHANGE_CONTROL_NUMBER, at(e, 1));
        ...
        case "AK5", "IK5" -> {
          ...
          addField(
              fields,
              RecordLevel.RECORD,
              X999.TRANSACTION_SET_CONTROL_NUMBER,
        ...
        default -> {
          /* other segments (GS/ST/AK3/AK4/IK3/IK4/SE/GE/IEA) carry no field we surface */
        }
      }
    }
  ...
  /**
   * The element at index {@code i}, trimmed; null when absent or blank (absence, not a blank
   * value).
   */
